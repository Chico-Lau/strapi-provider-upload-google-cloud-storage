  ...
  return {
    async upload(file) {
      try {
        ...
        console.debug(`File successfully uploaded (1) to ${file.url}`);
      } catch (error) {
    ...
    async uploadStream(file) {
      ...
      console.log('watermarkImage: ' + config.watermarkImage);
      console.log('watermarkPosition: ' + config.watermarkPosition);
      console.log('file,mime: ' + file.mime);
      
      if (config.watermarkImage && config.watermarkPosition && (file.mime == 'image/jpeg' || file.mime == 'image/png')) {
        const buffer = await streamToBuffer(file.stream);

        const watermarkImageWidth = Math.floor(file.width/5);
        const watermarkImage = await sharp(__dirname + '/img/' + config.watermarkImage).resize(watermarkImageWidth).toBuffer();

        file.stream = sharp(buffer).composite([{
          input: watermarkImage,
          gravity: config.watermarkPosition
        }]);
      }
      ...
      try {
        ...
        file.url = `${baseUrl}/${fullFileName}`;
        console.debug(`File successfully uploaded (2) to ${file.url}`);
      } catch (error) {
        // Re-throw so that the upload operation will fail
        // and error will surface to the user in the Strapi admin front-end
        console.error(`Error uploading file to Google Cloud Storage: ${error.message}`);
